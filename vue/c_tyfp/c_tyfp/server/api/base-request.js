...
var serverAddressURL = URL.parse(CanEditServerConf.EServer.address);
...
module.exports = {
  ...
  REQUEST: function (req, res, successFunc, errorFunc) {
    var userSession = req.cookies.tyfp || '';
    if (!APIWhiteConfig.includes(req.url)) {
      sessionManager.verifyLoginSession(userSession, function (isLogin) {
        if (isLogin) {
          ...
          // 未登录
          var resp_row = {resp_code: '200', resp_msg: '', biz_code: 'NODE_LOGIN', biz_msg: '未登录'};
          var baseResponse = new BaseResponse(resp_row);
          successFunc({resp_info: baseResponse});
        }
      });
    } else {
      doRequest(req, res, successFunc, errorFunc);
...
function doRequest(req, res, successFunc, errorFunc) {
  ...
  var requestOptions = {
    ...
    headers: {
      ...
      'X-sign': handlerCode.signRequest(req, JSON.stringify(req.body), APIServerConfig.TYFPServer.pramary_key),
      // 新token
  ...
  var APIServerRequest = APIServerHttp.request(requestOptions, function (response) {
    ...
    response.on('data', function (chunk) {
      ...
      if (this.statusCode !== 200) {
        ...
        var isVerifyOK = handlerCode.verifySign(RSPData, this.headers['x-sign'], APIServerConfig.TYFPServer.pramary_key);
        if (isVerifyOK) {

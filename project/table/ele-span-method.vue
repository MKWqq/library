<template>
    <el-table :data="data" :span-method="objectSpanMethod"></el-table>
</template>
<script>
    ...
    export default{
        ...
        methods:{
            ...
            // send ajax
            getLoadData: debounce(function () {
                let dataGridParams = this.$refs.datagrid.getQueryParams();
                // console.log(1);
                const _self = this;
                _self.$showViewLoading()
                HTTP.axios.post('/settleQuery/merchNoDetail', {
                    beginDate: _self.settleBeginDate,
                    endDate: _self.settleEndDate,
                    ...dataGridParams
                }).then((data) => {
                    _self.$closeViewLoading()
                    // console.log(data);
                    if (data.status === _self.$successStatus) {
                        // TODO get data
                        _self.data = data.object.records.tmp;
                        _self.spanMethodColumn = data.object.records.spanMethodObject.columns;
                        _self.spanMethodRow = data.object.records.spanMethodObject.rows;
                        _self.total = data.object.total;
                    }
                    else {
                        _self.$showFailureMsg(data.msg)
                    }
                }).catch(e => {
                    _self.$closeViewLoading()
                    _self.$showFailureMsg(e.message)
                });
            }, 200),
        }
    }

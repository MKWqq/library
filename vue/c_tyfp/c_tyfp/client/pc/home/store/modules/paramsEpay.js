...
export default {
  state: {
    ...
    // 缴费详情——缴费说明
    chargeIntroduce: '',
    /* 系统基本信息{cid:'',cid_lx:'',dl_dm:'',app_id:'',school_code:'',app_name:''} */
    basisInfo: null,
    /* 用户信息——查询账号 */
    businessInfo: null,
    /* 返回壳壳地址 */
  ...
  mutations: {
    ...
    SetActiveNav: function(state, payload) {
      state.ActiveNav = payload;
      if (window.localStorage) {
        window.localStorage.ActiveNav = state.ActiveNav;
      }
    },
    initActiveNav: function(state) {
      if (window.localStorage) {
        state.ActiveNav = window.localStorage.ActiveNav;
      }
    }
  },
  getters: {}
